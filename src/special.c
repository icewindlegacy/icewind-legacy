...
bool
spec_customs_weapons( CHAR_DATA *ch )
{
    OBJ_DATA *	obj;
    OBJ_DATA *	obj_next;
    CHAR_DATA *	victim;
	...
	for ( obj = victim->carrying; obj != NULL; obj = obj_next )
	{
	    ...
	    if ( obj->item_type == ITEM_WEAPON && can_see_obj( ch, obj ) )
	    {
		sayf( ch, "Weapons are banned from this area.  I'm going to have to confiscate %s.",
		      obj->short_descr );
		obj_from_char( obj );
		act_color( AT_ACTION, "$n confiscates $p`X from you.",
			   ch, obj, victim, TO_VICT, POS_RESTING );
		act_color( AT_ACTION, "$n confiscates $p`X from $N.",
...
spec_patrolman(CHAR_DATA *ch)
{
    ...
    &&	     IS_SET( obj->extra_flags, ITEM_WHISTLE ) )
...
spec_nasty( CHAR_DATA *ch )
{
	...
	case 0:
	    act_color( AT_ACTION,
		       "$n rips apart your coin purse, spilling your coins!",
		       ch, NULL, victim, TO_VICT, POS_RESTING );
	    act_color( AT_ACTION,
		       "You slash apart $N's coin purse and gather $S coins.",
			ch, NULL, victim, TO_CHAR, POS_RESTING );
	    act_color( AT_ACTION, "$N's coin purse is ripped apart!",
		       ch, NULL, victim, TO_NOTVICT, POS_RESTING );
	    amt.gold	= victim->money.gold / 10;
	    amt.silver = victim->money.silver / 10;
	    amt.copper	= victim->money.copper / 10;
	    amt.fract	= victim->money.fract / 10;
	    if ( CASH_VALUE( amt ) != 0 )
	    {
...
spec_resurrect( CHAR_DATA *ch )
{
    ...
    for ( victim = ch->in_room->people; victim != NULL; victim = victim->next_in_room )
        if ( victim != ch && IS_DEAD( victim ) )
            break;

    if ( victim == NULL )
        return FALSE;

    spell_resurrect( skill_lookup( "resurrect" ), ch->level, ch, (void *)victim, 0 );
...
spec_thief( CHAR_DATA *ch )
{
	...
	{
	    amt.gold	= victim->money.gold
			* UMIN( number_range( 1, 20 ), ch->level / 2 ) / 100;
	    amt.gold	= UMIN( amt.gold, ch->level * ch->level * 2 );
	    amt.silver = victim->money.silver
			* UMIN( number_range( 1, 20 ), ch->level/2 ) / 100;
	    amt.silver = UMIN( amt.silver, ch->level * ch->level * 8 );
	    amt.copper	= victim->money.copper
			* UMIN( number_range( 1, 20 ), ch->level / 2) / 100;
	    amt.copper	= UMIN( amt.copper, ch->level * ch->level * 24 );
	    amt.fract	= 0;
	    money_subtract( &victim->money, &amt, FALSE );
	    money_add( &ch->money, &amt, FALSE );
	    return TRUE;
