...
do_new_reboot( CHAR_DATA *ch, char *argument )
{
 ...
 if (arg[0] == '\0') 
    ...
    if (is_number(arg))
    {
     ...
     sprintf(buf,"Icewind Legacy will reboot in %i ticks.\n\r",reboot_counter);
     send_to_char(buf,ch);
     return;
    }
...
void reboot_mars( void )
{
    ...
    sprintf( log_buf, "Rebooting Icewind Legacy.");
    log_string(log_buf);
    for ( d = descriptor_list; d != NULL; d = d_next )
    {
	d_next = d->next;
	write_to_buffer(d,"Icewind Legacy is rebooting now!",0);
        if (d->character != NULL)
...
do_idle(CHAR_DATA *ch, char *argument)
{
	...
	if ( vch->desc && vch->desc->editor )
	    strncpy( status, olc_ed_name( vch ), 7 ), status[7] = '\0'; 
	else    
	    strncpy( status, vch->countdown > 0 ? "Quest" : "", 7 ), status[7] = '\0';
        ...
        sprintf(buf,"%-12s%4d%7d%5.1f %-8s%-30.30s\n\r",vch->name, vch->timer,
            ...
            (float)( vch->played + (int) (current_time - vch->logon) )/(3600*vch->level), 
                    status, vch->desc ? vch->desc->host : "Unknown"); 
        send_to_char(buf,ch);
    }
    if (number_percent() == 1 ) 
...
/* The first system line removes the oldest backup automatically */
   ...
   int result = system("rm ../../backup/legacy.tgz");  
   result = system("mv ../../backup/legacy3.tgz ../../backup/legacy4.tgz"); 
   result = system("mv ../../backup/legacy2.tgz ../../backup/legacy3.tgz");
   result = system("mv ../../backup/legacy1.tgz ../../backup/legacy2.tgz");
   result = system("mv ../../backup/legacy0.tgz ../../backup/legacy1.tgz");
   sprintf (buf, "tar zcvf ../../backup/legacy0.tgz ../../iwlegacy/ &");
   result = system (buf);
