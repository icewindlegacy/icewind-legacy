				 ...
				 bool hide, bool fKill)
{
	...
	if (damverb[0] == '\0')
	{
        if (sn >= 0 && sn < top_skill && skill_table[sn].noun_damage != NULL)
...
void kill_unit (CHAR_DATA * ch)
{
	...
	xSET_BIT(ch->affected_by, AFF_KNOCKOUT);
	remove_turn (ch->turn);

	act ("`PYou feel the sweet embrace of sleep eternal as your body "
...
void revive_unit (CHAR_DATA * ch)
{
	...
	xREMOVE_BIT(ch->affected_by, AFF_KNOCKOUT);
