...
typedef void SPELL_FUN	args( ( int sn, int level, CHAR_DATA *ch, void *vo,
				int target ) );
typedef void SONG_FUN   args( ( int songnum, int level, CHAR_DATA *ch,void *vo, int target ) );
 ...
 */
#define MUD_NAME		  "Icewind Legacy"
#define IMP_NAME		  "Mask"
#define GAME_PORT		  3033
#define BUILD_PORT		  3033
#define MUD_WEBSITE               "https://reaux.vineyard.haus"
...
#define MAX_SONGS  10 /* How many songs are in the game*/
...
#define PULSE_COMBAT              (20 * PULSE_PER_SECOND)
...
/*
 * Monster structure, for spell summon_monster
 */
struct monster_summon_type
{
    int		level;
    int		hit;
    int		move;
...
#define CON_ROLL_STATS   31
#define CON_CONFIRM_STATS 32
#define CON_GET_NEW_GOD 33
#define CON_MENU      34
...
struct skill_data
{
    ...
    int memorized;     /* NEW: # of times spell is memorized */
...
extern const int spell_slots[MAX_CHAR_LEVEL + 1][MAX_SPELL_LEVEL + 1];
...
#define KINGDOM_NEVERWINTER		1
#define KINGDOM_SILVERYMOON		2
#define KINGDOM_TEN_TOWNS		3
#define KINGDOM_LUSKAN		4
#define KINGDOM_EVERLUND		5
#define KINGDOM_ELTUREL		6
#define KINGDOM_MENZOBERRANZAN	7
/* #define MAX_KINGDOM		8 */
...
struct	char_data
{
    ...
    sh_int		level;
    /* Multiclass system */
    sh_int		multiclass_count;	/* Number of classes (1-3) */
    sh_int		class_levels[MAX_CLASS];	/* Level in each class */
    ...
    bool		riding;
    /* In CHAR_DATA */
    OBJ_DATA *nocked_arrow;   /* currently loaded arrow/bolt */
    bool crossbow_loaded; /* TRUE if crossbow is cocked and ready */
	COMBAT_DATA *      in_battle; /* Turn-Based Combat stuff, by Midboss. */
...
( can_see((looker), (ch)) ? \
    ( IS_IMMORTAL(ch) ? \
        (ch)->name : /* always show name for visible immortals */ \
        ( IS_NPC(ch) ? (ch)->short_descr : \
            ( knows_char((looker), (ch)) ? \
                (ch)->name : (ch)->short_descr ) ) ) \
  : ( IS_IMMORTAL(ch) ? "An Immortal" : "someone") )
...
/* multiclass.c */
void	init_multiclass	args( ( CHAR_DATA *ch ) );
void	recalculate_multiclass_count	args( ( CHAR_DATA *ch ) );
int	get_total_level	args( ( CHAR_DATA *ch ) );
int	get_class_level	args( ( CHAR_DATA *ch, int class_num ) );
bool	can_add_class	args( ( CHAR_DATA *ch ) );
bool	add_class	args( ( CHAR_DATA *ch, int class_num ) );
bool	is_skill_available_to_multiclass	args( ( CHAR_DATA *ch, int sn ) );
int	get_multiclass_skill_level	args( ( CHAR_DATA *ch, int sn ) );
bool	multiclass_meets_skill_level	args( ( CHAR_DATA *ch, int sn ) );
int	get_multiclass_skill_rating	args( ( CHAR_DATA *ch, int sn ) );
int	get_effective_class	args( ( CHAR_DATA *ch ) );
int	get_multiclass_exp_requirement	args( ( CHAR_DATA *ch, int target_level ) );
bool	is_ready_to_level	args( ( CHAR_DATA *ch ) );
void	show_multiclass_choices	args( ( CHAR_DATA *ch ) );
void	handle_multiclass_levelup	args( ( CHAR_DATA *ch, int choice ) );
