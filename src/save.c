...
fwrite_char( CHAR_DATA *ch, FILE *fp )
{
	...
	if ( !IS_NPC( ch ) )
	{
		...
		fprintf( fp, " %d", ch->class_levels[i] );
	    fprintf( fp, "\n" );
	}
...
/* Max supported nesting levels (same as ROM) */
#define MAX_NEST 100
...
void fwrite_obj_donation(OBJ_DATA *obj, FILE *fp, int iNest)
{
    ...
    /* dynamic strings (use ~ terminator like ROM) */
    ...
    for (paf = obj->affected; paf != NULL; paf = paf->next)
        fprintf(fp, "Affc %d %d %d %d %d\n",
                paf->where, paf->type, paf->level, paf->duration, paf->modifier);
    ...
    /* Now recurse into contents (important: object then its contents) */
    for (tobj = obj->contains; tobj != NULL; tobj = tobj->next_content)
        fwrite_obj_donation(tobj, fp, iNest + 1);
}
...
OBJ_DATA *fread_obj_donation(FILE *fp)
{
    ...
    {
        int i;
        for (i = 0; i < MAX_NEST; i++) rgObjNest[i] = NULL;
        fNestInit = TRUE;
    }
    ...
    for (;;)
    {
        ...
        if (!str_cmp(word, "End"))
        {
            if (!fVnum)
            {
                bug("fread_obj_donation: incomplete object.", 0);
                free_obj(obj);
                return NULL;
            }

            /* Link to parent if nested */
            if (fNest && iNest > 0 && rgObjNest[iNest-1] != NULL)
            {
                obj_to_obj(obj, rgObjNest[iNest-1]);
                /* Child consumed; we return NULL so caller doesn't place it in a pit/room */
                return NULL;
            }
            else
            {
                /* Top-level object; remember it in nest 0 and return to caller to place */
                rgObjNest[iNest] = obj;
                return obj;
            }
        }
        ...
        if (!str_cmp(word, "Nest"))
        {
            ...
            if (iNest < 0 || iNest >= MAX_NEST) iNest = 0;
            fNest = TRUE;
            /* prepare slot for this level */
            rgObjNest[iNest] = obj;
            continue;
        }
        ...
        if (!str_cmp(word, "WearLoc"))    { obj->wear_loc     = fread_number(fp); continue; }
        if (!str_cmp(word, "Weight"))     { obj->weight       = fread_number(fp); continue; }
        if (!str_cmp(word, "Timer"))      { obj->timer        = fread_number(fp); continue; }
        if (!str_cmp(word, "Cost"))       { obj->cost         = fread_number(fp); continue; }
        ...
        if (!str_cmp(word, "Affc"))
        {
            ...
            paf->location = APPLY_NONE; /* set if you save location */
        ...
        /* Unknown word: swallow the line to be forgiving */
        fread_to_eol(fp);
    }
}
...
load_char_obj( DESCRIPTOR_DATA *d, char *name )
{
    ...
    /* Initialize multiclass system for existing characters if not already set */
    /* Only initialize if multiclass_count is 0 AND class_levels array is empty */
    if ( found && !IS_NPC( ch ) && ch->multiclass_count == 0 )
    {
        int i;
        bool has_class_levels = FALSE;
        
        /* Check if any class_levels are set */
        for ( i = 0; i < MAX_CLASS; i++ )
        {
        ...
        /* Only initialize if no class levels are set */
        if ( !has_class_levels )
        {
            init_multiclass( ch );
        }
...
//	    KEY( "Cla",		ch->class,		fread_number( fp ) );
	    ...
	    /* Multiclass system loading */
	    if ( !str_cmp( word, "Mcls" ) )
	    {
		ch->multiclass_count = fread_number( fp );
		fMatch = TRUE;
		break;
	    }
	    if ( !str_cmp( word, "Pcls" ) )
	    {
		ch->primary_class = fread_number( fp );
		fMatch = TRUE;
		break;
	    }
	    if ( !str_cmp( word, "Scls" ) )
	    {
		ch->secondary_class = fread_number( fp );
		fMatch = TRUE;
		break;
	    }
	    ...
	    if ( !str_cmp( word, "Pend" ) )
	    {
		ch->pcdata->pending_class_choice = fread_number( fp ) != 0;
		fMatch = TRUE;
		break;
	    }
	    if ( !str_cmp( word, "Clvls" ) )
	    {
		int i;
		for ( i = 0; i < MAX_CLASS; i++ )
		    ch->class_levels[i] = fread_number( fp );
		fMatch = TRUE;
		break;
	    }
	    ...
	    if ( !str_cmp( word, "Skl" ) )
{
    ...
    int f_use;
    int l_use;
    int memorized = 0;  /* default for older files */
    char *temp;
    ...
    /* Try reading memorized, if next token is a number */
    temp = fread_word( fp );
    if ( is_number(temp) )
    {
        memorized = atoi(temp);
        temp = fread_word(fp);  /* read skill name next */
    }
    ...
    else
    {
        ...
        ch->pcdata->skill[sn].memorized   = memorized;
    }
    fMatch = TRUE;
    break;
}
	...
	if ( !fMatch )
	{
	    /* Don't report missing multiclass fields as bugs for old characters */
	    if ( str_cmp( word, "Mcls" ) && str_cmp( word, "Pcls" ) && 
	         str_cmp( word, "Scls" ) && str_cmp( word, "Tcls" ) && 
	         str_cmp( word, "Pend" ) && str_cmp( word, "Clvls" ) )
	    {
